 ...
 * Memoranda application GUI. It can be used to display all <code>Contact</code>
 * s within the application or to display <code>Contact</code>s associated with
...
public class ContactsTable extends JTable {
	...
	public ContactsTable(Type type) {
		super();
		this._type = type;
		final ContactsTableModel cModel = new ContactsTableModel();
		setModel(cModel);
		_initTable();
	...
	public TransferableContacts<Contact> getTransferableContacts() {
	...
	public void valueChanged(ListSelectionEvent event) {
		...
		final int[] indexes = this.getSelectedRows();
		for (int i = 0; i < indexes.length; i++) {
			final Contact c = (Contact) this.getModel().getValueAt(indexes[i], ContactsTable.CONTACT);
	...
	public class ContactsTableModel extends AbstractTableModel {
		...
		private String[] _columnNames = { Local.getString("First Name"), Local.getString("Last Name"),
				Local.getString("Email"), Local.getString("Telephone"), Local.getString("Organization") };
		...
		public Object getValueAt(int row, int col) {
			final Contact ev = (Contact) _contacts.get(row);
			if (col == 0) {
	...
	/**
	 * A transferable <code>ArrayList</code> of <code>Contact</code>s used by
	 * the UI for drag-n-drop functionality. This class is currently not being
	 * used for its full purpose until problems with selection are fixed to
	 * allow multiple contacts to be transfered.
	 * 
	 * @author Jonathan
	 *
	 */
	public class TransferableContacts<C extends Contact> extends ArrayList<C> implements Transferable {
		...
		public TransferableContacts() {
			super();
			try {
				_dataFlavor = new DataFlavor(
						DataFlavor.javaJVMLocalObjectMimeType + ";contact=" + TransferableContacts.class.getName());
			}
	...
	private class NameFilter extends RowFilter<ContactsTableModel, Integer> {
		...
		public boolean include(javax.swing.RowFilter.Entry<? extends ContactsTableModel, ? extends Integer> entry) {
			...
			final ContactsTableModel ctModel = (ContactsTableModel) entry.getModel();
			final int i = entry.getIdentifier().intValue();
			final Contact contact = (Contact) ctModel.getValueAt(i, ContactsTable.CONTACT);
			final String nameString = contact.getFirstName() + " " + contact.getLastName();
